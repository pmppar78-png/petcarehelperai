...
function generateFirstTimeContent(name, data, meta) {
  ...
  const careTeamVariants = [
    ...
    `No ${name} owner succeeds alone. Assemble your support team early: a primary ${meta.vetType} who knows this ${meta.groupLabel} inside and out, an emergency veterinary contact for after-hours crises, and a grooming professional who understands ${name}'s specific needs. ${ex === 'high' || ex === 'very-high' ? (meta.term === 'dog' ? `For an active ${meta.groupLabel} like ${name}, a dog walker or exercise companion for days when you cannot meet their full activity needs is worth the investment.` : `For an active ${meta.groupLabel} like ${name}, a backup care provider who understands daily species-appropriate enrichment is worth identifying early.`) : `Even with ${data.exerciseNeeds.toLowerCase()} exercise needs, having a backup person who can step in for daily care during illness or travel is essential.`} Pet sitter relationships take time to build—trial runs before actual need reveal compatibility issues. Fellow ${name} owners, both local and online, become your most practical resource for ${meta.groupLabel}-specific questions that professionals may not prioritize.`,
  ];
...
function generateHabitatContent(name, data, meta) {
  ...
  return `
      ...
      <p>The indoor versus outdoor question for ${name} depends on climate, safety, and this ${meta.groupLabel}'s specific environmental tolerances. ${name} ${meta.termPlural} with ${data.temperament.toLowerCase()} traits generally ${meta.term === 'dog' ? (ex === 'high' || ex === 'very-high' ? 'benefit from outdoor access for exercise and mental stimulation' : 'thrive primarily indoors with supplemental outdoor exposure') : 'do best when the primary habitat is stable and enrichment is planned around species-appropriate conditions'}. Indoor environments offer climate control, protection from predators and hazards, and closer monitoring of health. ${meta.term === 'dog' ? `If providing outdoor time for your ${name}, ensure the space is fully secured with species-appropriate fencing or enclosure, free from toxic plants or chemicals, and supervised at all times. Extreme weather conditions require bringing your ${name} indoors regardless of normal routine.` : `For non-dog species, "outdoor" exposure should be treated cautiously: temperature, humidity, predators, pathogens, and escape risk usually make controlled indoor husbandry the safer default.`} Many ${name} owners find that a combination approach—stable primary housing with supervised enrichment—provides the best balance of safety and stimulation.</p>
